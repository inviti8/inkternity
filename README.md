...
- **Particle effects (TimelineFX)** — drop a `.tfx` effect package (authored in the external [TimelineFX editor](https://www.rigzsoft.co.uk/)) onto the canvas to embed a live, animated particle system: rain, snow, embers, dust, magic, explosions. It plays in author mode and animates in reader mode, moves/scales/rotates like any object, and is rendered faithfully through a Skia runtime-shader port of the TimelineFX fragment shader (per-texel color-ramp gradient mapping). The package — including its textures — is embedded in the save; host-authored in collaborative sessions
