...
- **Flip-book layer groups + motion paths** — turn a layer folder into an animation: each child layer is a frame, played one at a time with tunable fps, play style (once / loop / ping-pong) and trigger (auto-on-view / on-touch, mirroring the particle modes), invert direction, and an editing onion skin that ghosts the frames below/above the one you're on. Playback runs in reader mode (with a drawing-mode Preview toggle for timing). A group can also **travel a drawn bezier motion path** as it plays — draw and curve the path like a shape, then per node set the leg's duration in seconds, a scale, easing, and a rotation-along-tangent factor (orient to the curve), all tweened; the path has its own play style so the frames can loop while the body strides the path once. A fourth Folder Mode, **Anim-FX**, sends the particle effects inside a folder travelling that same motion-path type with real secondary motion — the emitter moves while its particles trail behind (embers off a moving torch, a comet, a magic sweep), with multiple effects keeping their painted arrangement. Folder Mode (Normal / Parallax Scene / Flip-Book / Anim-FX) is one dropdown since a folder is at most one; nest to combine
...
- **Stylus-friendly layer rows** — one-tap buttons on every layer/folder row (no precise row-click or double-click needed with a pen): **Select** (also reaches a folder's mode settings), **Up/Down** to reorder in place, and **Duplicate** — a true deep copy carrying all content, blend/alpha, parallax depth, and a folder's mode + motion path + whole subtree
...
- **3D reference figures (armature poser)** — drop a posable humanoid mannequin on the canvas from the top toolbar and double-click to open a lightweight 3D editor: orbit/pan/zoom camera tools, lighting, FK joint posing with on-screen red/green/blue axis handles, a height slider plus shape-key sliders (gender, build, head/eye/nose/mouth shape, expressions), per-material colors, and an adjustable FOV with a perspective⇄orthographic lens toggle. Bake captures the posed figure to the canvas as a re-editable image (double-click any time to re-pose). **Load Model** imports your own glTF/`.glb` — a skeleton matching the built-in rig loads fully posable, anything else loads as a flattened static reference mesh; the model is embedded in the save so it travels with the file. Simple flat shading for now (no textures or animation; Draco-compressed files are rejected with a re-export hint). **Reusable per-user presets**: save a **Character** (body look) or a **Scene** (camera+light+lens) and apply it across any file from a thumbnailed tile list. Inkternity's first 3D subsystem, built on a raw-GL pass composited back through Skia
- **AI reangle (style-preserving camera turn)** — pick a character drawing, open the orbit editor, and turn the camera a little *without redrawing*: the linework is captured, a rough 3D proxy is reconstructed on a warmed GPU, and your original pixels are re-projected onto it at the new angle. The drawing is moved, never regenerated — your style is preserved exactly. Runs on a self-hosted AI service (hvym-img-tools), not a third-party cloud
- **AI 3D reference (sketch → orbitable mesh)** — turn a rough sketch into an untextured, orbitable 3D reference to draw over; it lands on its own layer above your drawing, shaded gray from surface normals, and you keep drawing while you orbit it. Same self-hosted service
- **AI warm-time billing (wallet-native)** — the AI tools bill for GPU *warm time* rather than per image, paid peer-to-peer in USDC from your own self-custodied Stellar wallet (the same DevKeys wallet C2PA uses). Toggle **AI inference** in the top bar; a one-time consent card shows the rate (about $0.05/min) and then each warm window is bought automatically via an on-chain x402 pay-per-window handshake as you work. No accounts, no fiat, no server-held balance, no free tier — crypto-rails-gated exactly like verifiable publishing
