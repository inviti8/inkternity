...
M3 Character tab + M4 docs all landed, **plus M5: Pose presets** (save/load named
poses by bone name, added to the existing Pose tab — same machinery, third preset
kind). Follow-on to PHASE9 (the armature poser + external model loading, see
[PHASE9.md]). Adds two new tabs to the armature editor —
