# PHASE 9 — "New Publications" lobby carousel

## Status

**SCOPED — not started.** First half of the PHASE9/10 split (the other half is
[PHASE10.md], the 3D armature poser). This phase is the small, low-risk one:
ships a promotional carousel into the lobby to cross-pollinate Inkternity
artists' works. No save-format bump (touches the lobby, not canvas files).
