...
- Rectangle tool
    ...
    - **Curves** (polygon): select a **single vertex** and press **Make Curve** to turn its edges into bezier curves — green tangent handles (with arms) appear; drag them to shape the curve. **Make Corner** reverts to straight. A curve node is **Smooth** by default (the two tangents mirror for a continuous curve); **Make Cusp** lets the tangents move independently for a sharp transition, **Make Smooth** re-links them. Add Point on a curved edge splits the curve without changing its shape. Mix straight and curved edges freely — masks, flatten, and export all follow the curves
...
## Audio Cues
- Per-waypoint audio: attach an mp3 to any waypoint by **dragging the file onto the canvas while that waypoint is selected**. The Waypoint tool's Audio block on the right-hand panel shows the attached file name, a Clear button, a Loop checkbox, and the canvas-wide budget readout
- Reader mode plays the attached clip on arrival. Author mode is silent — audio only fires while reader mode is active
- **Audio resolution** walks the waypoint graph: at each waypoint the reader visits, if it has audio attached, that clip plays from the beginning; if it has a "Stop audio here" flag, audio cuts to silence; otherwise the rule walks back through the first incoming edge until it finds an audio-bearing waypoint (or a stop anchor). That means a chain of waypoints can share a single audio track — only the FIRST one in the chain needs the mp3 attached
- **Hard cuts on swap**: arriving at a new audio-bearing waypoint stops the current clip and starts the new one from the beginning. Arrival on a "Stop audio here" waypoint cuts to silence. Audio continues across waypoints that have nothing audio-related set
- **Audio fires on arrival, not on button-press** — when you press Forward / Back / a branch button, the current clip keeps playing through the smooth-camera pan; the new clip (or silence) kicks in when the camera arrives at the destination. Pressing buttons rapidly rolls the timer forward to the final destination, so you never hear stale clips
- **Back through history replays from the predecessor** — pressing Back doesn't resume audio where it was; it re-applies the audio rule from the back-target. If that waypoint resolves to clip X, X starts from the beginning. Predictable: every waypoint plays its audio the same way every time you arrive there
- **Stop audio here**: an independent flag on every waypoint — set it without attaching any clip and that waypoint becomes a "silence anchor" in the chain. Useful for ending a track at a specific narrative beat. If both an mp3 and Stop audio are set on the same waypoint, the mp3 wins
- **Cumulative cap: 30 MB per canvas.** Sum of all attached mp3s. The Audio block on the panel shows the running total ("Canvas audio: N.N MB / 30 MB"). Attaching a file that would push the canvas over the cap is refused with an error log; the file is not added. Dedup-aware: N waypoints referencing the same mp3 cost 1× its size against the budget. ResourceManager handles serialisation — the mp3 bytes ship with the `.inkternity` file, so a recipient who opens it standalone hears the cues without needing the source mp3
- **Subscription / live collab** sub-side playback is **not in this release**. The mp3 bytes still sync to subscribers via the existing resource path, but `ReaderMode` is per-client; only the artist hears the cues during a live presentation. For audience-with-audio sessions: either record the host's screen externally (OBS captures the audio too) or hand the `.inkternity` file to the audience for local playback
- **Marker glyphs**: a small blue dot in the upper-right of a waypoint marker means "audio attached"; a grey dot means "silence anchor (Stop audio here)." Both vanish in reader mode (editor chrome only)
## Reader Mode
...
- Select a layer by clicking it once. When it is selected, you can change its properties such as name, alpha, blend mode, and parallax depth — and, when a **folder** is selected, its **Parallax Scene** settings (see *Parallax Scenes* below). This is different from the "set to edit" mode
...
- **Row buttons (stylus-friendly)** — every layer/folder row carries a set of one-tap buttons on the right, so you never have to precisely click or double-click the row itself (handy with a pen):
    - **Select** (cursor icon) — selects the row and, for a real layer, makes it the edit target in one tap. Works for **folders** too, so a folder's Parallax Scene / Flip-Book / Anim-FX settings are reachable without a double-click
    - **Up / Down arrows** — move the layer one slot up or down within its folder. (Reordering this way is not added to the undo stack — it's a quick nudge; use drag-sort if you prefer)
    - **Duplicate** (two-squares icon) — makes a full copy of the layer or folder and drops it in right below. It's a true deep copy: all content, alpha/blend, parallax depth, and a folder's mode (Parallax / Flip-Book / Anim-FX) + motion path come along, and a folder copies its whole subtree. Undoable
    - The same Up / Down / Duplicate / visibility (and, for a normal layer, Delete) buttons are also mirrored as a row at the **top of the layer properties panel**, so you can reach them from the larger settings controls when a layer is selected
- **Merge Down** (button in the layer properties panel) moves everything on the selected layer into the layer below it — on top of that layer's existing content — and removes the now-empty layer. The merge is lossless: vector strokes stay vector, text stays editable, pixel strokes stay individually erasable. Fully undoable (three undo steps). To keep the merge visually exact, it refuses (with an explanation) when either layer has non-default alpha/blend or parallax depth, when the layer holds waypoint markers, when the item below is a folder, or for the named Sketch/Color/Ink layers (merge *into* them instead)
...
-------------------------------------------------------------------------------------------
## Parallax Scenes (Multiplane Camera)
A parallax scene is a **layer folder** plus the **depths** of the layers inside it. The folder anchors the scene to a chosen viewpoint and scale; the layers inside slide past each other as the camera moves. Setup is three steps:

1. **Group your layers in a folder.** Put the layers that make up the scene (e.g. "sky", "hills", "foreground") into one folder.
2. **Frame the scene, then mark the folder as a Parallax Scene.** Move the camera so the scene is composed exactly how it should look at rest, then select the folder and tick **Parallax Scene** in the layer properties panel. This captures the current camera as the scene's **neutral viewpoint** (its anchor position and reference scale). Capturing never moves anything on screen — at the neutral viewpoint every layer sits exactly where you composed it.
3. **Give each layer a Depth.** Select a layer inside the folder and set its **Parallax Depth** (slider under Blend Mode). `0` = the canvas plane; positive = farther away (slides less as you pan); negative (down to −0.9) = nearer than the canvas (slides more, good for foreground overlays). Layers at different depths drift past each other as you pan — the classic multiplane effect.

Folder-panel controls (shown when the folder is a Parallax Scene):
- **Set Anchor and Scale to Current View** — re-capture the neutral viewpoint from wherever the camera is now (use it to re-frame the scene).
- **View vs scene scale: N.NN×** — a live readout of how far the current zoom is from the scene's neutral scale (`1.00` = at neutral; above = zoomed out, below = zoomed in).
- **Go to Neutral View** — smoothly flies the camera back to the scene's neutral viewpoint.

- **Scale-aware.** The parallax effect is referenced to the scale you captured, so it stays sensible no matter how far you zoom: it **diminishes as you zoom out** (so a parallax scene tucked inside a small picture on a wall reads as a flat picture from across the room) and **stops growing once you zoom in past the scene's scale** (no runaway separation). Zoom itself never produces parallax — only camera movement does
- **Works everywhere the art renders** — on-canvas, screenshots, SVG export, and **reader mode / waypoints**. Because the scene is anchored, a waypoint placed at the neutral viewpoint shows the scene at rest, and the parallax settles into place as the camera travels there
...
### Anim-FX Groups — particle effects that travel a path
Set **Folder Mode → Anim-FX** to make the **particle effects** inside a folder travel a motion path *with real trailing* (embers streaming off a moving torch, a comet, a magic sweep). Unlike a flip-book (which moves its content rigidly), an Anim-FX group drives each effect's simulation along the path, so particles lag behind the moving emitter the way they should.

1. Place one or more **particle effects** on layers inside a folder (they keep their painted positions relative to each other — the whole arrangement travels together).
2. Select the folder → **Folder Mode → Anim-FX** → **Add Motion Path**, and draw/curve the path (per-node **Seconds**, **Play Style**, and — from the motion path — Scale/Rotation all apply to the travel).
3. **Preview Animation** plays it in drawing mode; otherwise it runs in reader mode.

Notes: a trail only appears for effects authored with **non-relative** particles (most trailing/spray effects are); a purely local effect will slide rigidly. A **long** path spreads the trail over a large area (more redraw) — best for short/medium moves. If an effect's particles get cropped at its edges, raise its **Bounds size** on the particle brush (below) before placing it.
## Flatten Layer
...
- The bake never goes coarser than the finest ink stroke's own pixels (so pixel art stays crisp). For **vector** content, which has no native resolution, **zoom in before flattening to preserve more detail**. Very large regions are capped by **Settings->General->Maximum flatten size (px per axis)** (default 8192; raising it costs memory) and a notice appears if the result had to be downsampled
...
- **Show performance metrics** (Menu->Settings->Debug): a live overlay (FPS, frame-time breakdown, draw counters) for diagnosing slowdowns. Off by default; free when off
- **Motion cache coarsen shift** (Menu->Settings->Debug, default **0 = off**): a last resort for **low-end machines** that can't keep large layers resident in GPU memory. When set above 0, the canvas renders at reduced resolution *while the camera is moving* (a blurry-but-smooth pan), snapping back to full quality the moment it stops. Higher values are smoother but blurrier in motion. Most machines should leave this at 0 — raise the GPU resource cache budget instead, which preserves quality
### Best practice for large projects
- **Keep distinct sections/scenes on their own, spatially-compact layers, and flatten per layer.** A flattened layer becomes one image, so a layer whose artwork is packed into a small area bakes crisp and stays small in memory. A single layer spanning a huge area (e.g. several far-apart scenes on one layer) is the anti-pattern: when flattened it has to **downsample to the size cap** (blurry) and the resulting texture is large. Organizing by layer keeps flattened images both sharp and VRAM-friendly
- Flattening is safe against runaway memory: any one flattened image is capped by **Maximum flatten size**, and the bake aborts with a notice (rather than crashing) if it can't allocate. The thing to watch is having *many* large flattened layers visible at once on a low-VRAM GPU — keep the GPU resource cache budget at or below your card's memory, and prefer compact per-layer sections
## Particle Effects (TimelineFX)
- Author particle effects in the external **[TimelineFX editor](https://www.rigzsoft.co.uk/)** (rain, snow, embers, dust, magic, fire, explosions) and export a **`.eff` library**, then bring it into Inkternity with **File ▸ Import FX Library** (or just **drag the `.eff` onto the canvas**). One library can hold many effects
- Open the **FX Library** panel (top toolbar) and pick the **active effect** (single selection). Then choose the **Particle Brush** (magic-wand icon) and **paint** instances onto the layer you're editing — click to place one, or hold/drag to scatter a trail. The brush has **Size** and **Rate** sliders, a **Bounds size** slider, and a **Play on touch** toggle. **Bounds size** enlarges an effect's clip/selection box: if a wide or fast effect's particles look cropped at the edges, raise it before placing (it costs a little extra redraw area, so leave it at 1 unless an effect needs it)
- Each placement plays in author mode and **animates in reader mode** too, so atmospheric effects breathe while a reader sits on a panel. It honors the effect's own **Continuous/Finite** setting: looping effects (snow, embers) run whenever their layer is visible; one-shots (explosions) play when they come into view — including across **parallax layers** and **reader-mode waypoint navigation** — or wait for a tap if **Play on touch** is set. Move, scale, and rotate placements with the Edit tool like any other object
- The imported library — including its textures — is **embedded once in your `.inkternity` save** and shared by every placement, so the file stays self-contained and the effects travel with it
...
- **Reusable presets (Character & Scene tabs).** Save setups once and reuse them across *every* file/project — they're stored per-user, not inside the canvas. The **Scene** tab saves a **camera + light + lens** setup; the **Character** tab (built-in mannequin) saves a **body look** (height + material colors + shape keys). Type a name, hit Save, and the preset appears as a **thumbnail tile** — click a tile to apply it to the current figure/view, or the **×** to delete. Great for keeping a consistent lighting setup or a cast of recurring character builds
