...
- Performance note: while any visible layer has a non-zero depth, the canvas redraws without its usual caching. For heavy layers (lots of ink strokes), run **Flatten Layer** on them first — a flattened layer is a single image and parallax-pans for free
## Flatten Layer
