...
// transfer. curl keeps pointers into `mime`/`headers`/the field buffers, so they
// must outlive the handle — this struct owns them and is freed only after
// curl_multi_remove_handle + curl_easy_cleanup.
struct Handler {
    CURL* easy = nullptr;
    curl_mime* mime = nullptr;
    curl_slist* headers = nullptr;
    std::shared_ptr<ToolClient::Request> req;
    std::string toolName;   // for messages/logging
    // Stable backing store for the multipart fields: curl_mime_data reads lazily
    // (CURL_ZERO_TERMINATED for text, explicit length for files), so the bytes
    // these Fields own must stay put until cleanup. Owned here by value.
    std::vector<ToolClient::Field> fields;
};
...
// Capture the headers we branch on: X-Cache (was it a cache hit?), X-Tool-Version
// (which pipeline built the mesh), and X-Cache-Key (the content address / library
// asset id). Case-insensitive name match.
size_t header_callback(char* buffer, size_t size, size_t nitems, void* userp) {
    auto* req = static_cast<ToolClient::Request*>(userp);
...
int xfer_callback(void* clientp, curl_off_t dltotal, curl_off_t dlnow,
    ...
    auto* req = static_cast<ToolClient::Request*>(clientp);
...
std::string message_for_http(const std::string& tool, long code,
    ...
    switch (code) {
        case 401: return "The " + tool + " service rejected the API key (check configuration).";
...
// The .glb ASCII magic. Both current tools return model/gltf-binary; validating
// before load keeps a JSON error body (on an unexpected 200-with-wrong-body path)
// out of the mesh loader (REANGLE_API.md §6).
...
void finish(Handler* h, CURLcode result) {
    ToolClient::Request& req = *h->req;
    if (result == CURLE_OK) {
        curl_easy_getinfo(h->easy, CURLINFO_RESPONSE_CODE, &req.httpCode);
        if (req.httpCode == 200 && looks_like_glb(req.glb)) {
            req.status = ToolClient::Request::Status::SUCCESS;
        } else if (req.httpCode == 200) {
            req.error = "The " + h->toolName + " service returned a non-model response.";
            req.glb.clear();
            req.status = ToolClient::Request::Status::FAILURE;
        } else {
            req.error = message_for_http(h->toolName, req.httpCode, req.glb);
            ...
            // is surfaced on the main thread by the flow's tick via req.error.
            Logger::get().cross_platform_println(
                "[" + h->toolName + "] HTTP " + std::to_string(req.httpCode) + ": " +
        ...
        req.error = "The " + h->toolName + " request failed: " + curl_easy_strerror(result);
        req.glb.clear();
        req.status = ToolClient::Request::Status::FAILURE;
    }
}
...
void worker_update() {
    ...
    for (;;) {
        ...
        if (mc != CURLM_OK) {
            Logger::get().cross_platform_println("[toolclient] curl_multi failed; worker stopping.");
            break;
        }
        int numfds = 0;
    ...
    for (Handler* h : gCurrent) {
        h->req->error = "Request cancelled (application closing).";
        h->req->status = ToolClient::Request::Status::FAILURE;
        curl_multi_remove_handle(gMulti, h->easy);
        free_handler(h);
    }
    gCurrent.clear();
...
std::shared_ptr<ToolClient::Request> ToolClient::request(
    ...
    if (baseUrl.empty() || apiKey.empty()) {
        req->error = "The " + toolName + " tool is not configured (set the endpoint URL and API key).";
        req->status = Request::Status::FAILURE;
        return req;
    }
    if (fields.empty()) {
        req->error = "Nothing to send to the " + toolName + " tool (no fields).";
        req->status = Request::Status::FAILURE;
        return req;
    }
    ...
    h->req = req;
    h->toolName = toolName;
    h->fields = std::move(fields);   // own the field bytes for the transfer's lifetime

    h->easy = curl_easy_init();
    if (!h->easy) {
        free_handler(h);
        req->error = "The " + toolName + " request could not be created (curl init failed).";
        req->status = Request::Status::FAILURE;
        return req;
    }
    ...
    url += "/tools/" + toolName;   // the proxy routes by tool name → RunPod endpoint
    ...
    for (const Field& f : h->fields) {
        curl_mimepart* part = curl_mime_addpart(h->mime);
        curl_mime_name(part, f.name.c_str());
        if (!f.bytes.empty()) {   // file part
            if (!f.filename.empty()) curl_mime_filename(part, f.filename.c_str());
            if (!f.mimeType.empty()) curl_mime_type(part, f.mimeType.c_str());
            curl_mime_data(part, reinterpret_cast<const char*>(f.bytes.data()), f.bytes.size());
        } else {                  // text part
            curl_mime_data(part, f.textValue.c_str(), CURL_ZERO_TERMINATED);
        }
    }
    ...
    curl_easy_setopt(h->easy, CURLOPT_USERAGENT, "Inkternity-Tools/1.0");
...
std::shared_ptr<ToolClient::Request> ToolClient::request(
        const std::string&, const std::string&, const std::string& toolName,
        std::vector<Field>) {
    auto req = std::make_shared<Request>();
    req->error = "The " + toolName + " tool is not available in the web version.";
    req->status = Request::Status::FAILURE;
    return req;
}
